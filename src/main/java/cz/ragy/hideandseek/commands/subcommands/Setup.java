...
public class Setup extends SubCommand {
    ...
    public void perform(Player player, String[] args) {
        if(args.length > 7 && args[1].equals("arena")) { player.sendMessage(PlaceholderAPI.setPlaceholders((OfflinePlayer) player,Colors.translate(prefix + invalidMessage))); return; }
        if (args.length == 2) {
            if (args[1].equals("lobby")) {
                player.sendMessage(PlaceholderAPI.setPlaceholders((OfflinePlayer) player, Colors.translate(lobbySet)));
                ...
                if (!LobbySpawnStatus) {
                    player.sendMessage(PlaceholderAPI.setPlaceholders((OfflinePlayer) player, Colors.translate(LobbyWarning)));
                }
                ...
                if (args[1].equals("arena")) {
                    if (!player.hasPermission("has.createarena") || !player.hasPermission("has.*")) { player.sendMessage(Colors.translate(noPerms)); return; }
                    ...
                    if(!(digit.containsDigits(args[2])) &&
                        ...
                        player.sendMessage(PlaceholderAPI.setPlaceholders((OfflinePlayer) player, Colors.translate(prefix + invalidMessage)));
                        return;
                    }
                } else {
                    player.sendMessage(PlaceholderAPI.setPlaceholders((OfflinePlayer) player,Colors.translate(prefix + invalidMessage)));
